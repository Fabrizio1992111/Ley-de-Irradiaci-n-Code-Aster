SUBROUTINE UMAT(STRESS, STATEV, DDSDDE, SSE, SPD, SCD, RPL,           &
  ...
  real(8),parameter               :: tolerNR = 1.0d-04           ! Tolerancia admisible para N - R
  ...
  integer,parameter               :: iNRmax = 500                ! Numero de iteraciones maximas
  ...
  !----------------------------------------------------------------------------------------------------------------------------------------
   ...
   DPA = PROPS(3)
   !IRRA=PROPS(3)
   IF (TIME(2) + DTIME < 0.0d0) THEN
    IRRA = 0.0d0
   ELSE
    IRRA = DPA * ((TIME(2) + DTIME)) * 0.01d0 !Divide por el intervalo de tiempo [-1,10] 1/11=0.09=0.1
   ENDIF
...
IF (umbrals > umbral) THEN
    XresV(8) = vVs(8) - (Cf) * smises * IRRA*0.5d0
...
! residuo creep irradiacion
  ...
  XresV(9) = vVs(9) - H*(Ai*(umbrals-umbral))
